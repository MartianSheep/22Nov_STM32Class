...
def picture2Lines(picture, output, num, white = False) :
    ...
    with open(output,'w') as f :
        f.write(f"#define LED_WIDTH {num}\n")
        f.write(f"#define LED_NUM {LED_NUM}\n")
        f.write("\n")

        f.write('uint8_t LEDData')
        f.write('[' + str(num) + ']')
        f.write('[' + str(LED_NUM) + ']')
        f.write('[3]')
        ...
        for i in range(num) :
            ...
            for j in range(LED_NUM) : # LED number
                ...
                if (not is_white(pixel)) or white: # remove white
                    f.write('{' + str(pixel[1]) + ',' + str(pixel[0]) + ',' + str(pixel[2]) + '}')
                else :
                    f.write('{0,0,0}')
    ...
    with open(output,'w') as f :
        ...
        for stripe in range(num) :
            ...
            for led in range(LED_NUM) :
                ...
                f.write('{' + str(result[1]) + ',' + str(result[0]) + ',' + str(result[2]) + '}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', dest="num", type=int, default=60, help='number of stripes.')
